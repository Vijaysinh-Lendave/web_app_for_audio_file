...
def create():
    ...
    if str(f['#codec'])=='mp3':
        if str(request.form['option'])=='song':
            new_file= Song(name_of_song=str(f['tracktitle']),duration=int(f['#length']))
            db.session.add(new_file)
            db.session.commit()
            return render_template('index.html',statement=statement) ,os.remove(files[0])
            
        elif str(request.form['option'])=='podcast':
            new_file= Podcast(name_of_podcast=str(f['tracktitle']),duration=int(f['#length']),host=str(f['album']))
            db.session.add(new_file)
            db.session.commit()     
            return render_template('index.html',statement=statement),os.remove(files[0])

        elif str(request.form['option'])=='audiobook':
            new_file=Audiobook(title_audiobook=str(f['tracktitle']),author_title=str(f['composer']),narrator=str(f['artist']),duration=int(f['#length']))
            db.session.add(new_file)
            db.session.commit()  
            return render_template('index.html',statement=statement),os.remove(files[0])
    else:
